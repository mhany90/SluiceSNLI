...
def make_model(word_index, max_seq, embeds, embed_dim, ACTIVATION, L2, DP, SENT_REP_SIZE, LAYERS,  NO_TAGS):
   ...
   # #projection layer to reduce complexity
    #translate_1 = TimeDistributed(Dense(SENT_REP_SIZE,activation=ACTIVATION))

    #prem_embedded_sequences = translate_1( prem_embedded_sequences)
    #hypo_embedded_sequences = translate_1(hypo_embedded_sequences)
    ...
    BiLSTMenc_2 = Bidirectional(LSTM(SENT_REP_SIZE, activation="relu", dropout=DP, recurrent_dropout=DP,
                              return_sequences = True))
    #tag
    BiLSTMtagp_1 = Bidirectional(LSTM(SENT_REP_SIZE, activation="relu", dropout=DP, recurrent_dropout=DP,
                              return_sequences = True))
   # BiLSTMtagp_2 = Bidirectional(LSTM(SENT_REP_SIZE, activation="relu", dropout=DP, recurrent_dropout=DP,
   #                           return_sequences = True))
    BiLSTMtagh_1 = Bidirectional(LSTM(SENT_REP_SIZE, activation="relu", dropout=DP, recurrent_dropout=DP,
                               return_sequences = True))
  #  BiLSTMtagh_2 = Bidirectional(LSTM(SENT_REP_SIZE, activation="relu", dropout=DP, recurrent_dropout=DP,
   #                           return_sequences = True))

    #apply
    prem_hidden_1 = BiLSTMenc_1(prem_embedded_sequences)
    prem_hidden_2 = BiLSTMenc_2(prem_embedded_sequences)
    
    hypo_hidden_1 = BiLSTMenc_1(hypo_embedded_sequences)
    hypo_hidden_2 = BiLSTMenc_2(hypo_embedded_sequences)

    prem_tag_1 =  BiLSTMtagp_1(prem_embedded_sequences)
#    prem_tag_2 =  BiLSTMtagp_2(prem_embedded_sequences)

    hypo_tag_1 =  BiLSTMtagh_1(hypo_embedded_sequences)
 #   hypo_tag_2 =  BiLSTMtagh_2(hypo_embedded_sequences)

    #alphas 
    alpha_prem_hidden = WeightedCombinationLayerUnb()([prem_hidden_2, prem_tag_1])
    #alpha_tag_prem = WeightedCombination()([prem_hidden_1,prem_hidden_2, prem_tag_1])

    alpha_hypo_hidden = WeightedCombinationLayerUnb()([hypo_hidden_2, hypo_tag_1])
    #alpha_tag_hypo = WeightedCombination()([hypo_hidden_1, hypo_hidden_2, hypo_tag_1])
    ...
    hypo_out_tags = TimeDistributed(Dense(NO_TAGS, activation='softmax'))(hypo_tag_1)
     ...
     #shortcut
    alpha_prem_hidden = keras.layers.concatenate([alpha_prem_hidden, prem_hidden_1], axis = -1)
    alpha_hypo_hidden = keras.layers.concatenate([alpha_hypo_hidden, hypo_hidden_1], axis = -1)
